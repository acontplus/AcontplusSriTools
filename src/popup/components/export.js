// Componente de exportación para el popup de Acontplus SRI Tools v1.4.1
...
class ExportComponent {
  ...
  exportSelected() {
    ...
    const facturasSeleccionadas = this.manager.dataManager.facturas.filter(f => this.manager.dataManager.selectedFacturas.has(f.id));
  ...
  exportToExcel(facturas) {
    ...
    wb.Props = {
      ...
      Comments: 'Generado por Acontplus SRI Tools v' + this.manager.version + ' - Páginas: ' + this.manager.dataManager.paginationInfo.current + '/' + this.manager.dataManager.paginationInfo.total,
      CreatedDate: new Date()
    };

    const fecha = new Date().toISOString().split('T')[0];
    const hora = new Date().toTimeString().split(' ')[0].replace(/:/g, '');
    const paginaInfo = this.manager.dataManager.paginationInfo.total > 1 ? '_Pags' + this.manager.dataManager.paginationInfo.current + '-' + this.manager.dataManager.paginationInfo.total : '';
