  ...
  renderTable() {
    ...
    const tableHTML = this.manager.dataManager.facturas.map((factura, index) => {
      ...
      <tr class="${(this.manager.dataManager.selectedFacturas.has(factura.id) ? 'selected' : '')}"
          ...
          <td class="px-6 py-3 text-center">${factura.verificado === true ? '✔️' : factura.verificado === false ? '❌' : ''}${factura.tienePdf ? ` <span class="pdf-icon cursor-pointer hover:bg-gray-100 p-1 rounded" data-factura-id="${factura.id}" title="Abrir PDF">📄</span>` : ''}</td>
      </tr>`;
    }).join('');
